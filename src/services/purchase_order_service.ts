...
const createPurchaseOrder = (req: Request, res: Response, next: NextFunction) => {
   const { date, transactionDate, transactionType, shippingAddress,  status, companyId } = req.body;

   const PurchaseOrder = new purchase_order_model(
      {
         date, transactionDate, transactionType, shippingAddress, status, companyId
      })
